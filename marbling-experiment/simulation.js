...
function calcNumParticles(width, height) {
  return Math.min(
    Math.ceil(width * height * PARTICLE_DENSITY),
    MAX_NUM_PARTICLES
  );
}
...
const renderParticles = new GPUProgram(composer, {
  ...
  fragmentShader: `
        ...
        void main() {
            float ageFraction = float(texture(u_ages, v_uv_position).x) / ${PARTICLE_LIFETIME.toFixed(
              1
            )};
...
const touchVelocity = new GPUProgram(composer, {
  ...
  fragmentShader: `
        ...
        void main() {
            ...
            // Update velocity.
            vec2 velocity = texture(u_velocity, v_uv).xy + radiusSq * u_vector * ${TOUCH_FORCE_SCALE.toFixed(
              1
            )};
...
const touchMaterial = new GPUProgram(composer, {
  ...
  fragmentShader: `
        ...
        void main() {
            ...
            // Update material.
            out_material = clamp(texture(u_material, v_uv).x + radiusSq * ${ADD_MATERIAL_SCALE.toFixed(
              1
            )}, 0.0, 1.0);
        }`,
});
...
export function stepTouch(current, last) {
  if (last) {
    if (current[0] === last[0] && current[1] === last[1]) return;
    touchVelocity.setUniform("u_vector", [
      current[0] - last[0],
      -(current[1] - last[1]),
    ]);
...
canvas.addEventListener("mousemove", (event) => {
  ...
  stepTouch([x, y], lastMousePosition);
...
canvas.addEventListener("touchmove", (event) => {
  ...
  for (let i = 0; i < touches.length; i++) {
    ...
    stepTouch([x, y], lastTouchPositions[touch.identifier]);
    lastTouchPositions[touch.identifier] = [x, y];
  }
});
canvas.addEventListener("touchstart", (event) => {
  ...
  for (let i = 0; i < touches.length; i++) {
    const touch = touches[i];
    lastTouchPositions[touch.identifier] = [touch.clientX, touch.clientY];
  }
});
canvas.addEventListener("touchend", (event) => {
  ...
  const touches = event.changedTouches;
  for (let i = 0; i < touches.length; i++) {
    delete lastTouchPositions[touches[i].identifier];
  }
});
onResize();
setInterval(() => {
...
// module.exports = { PARAMS, onResize, stepTouch, stepSimulation };
